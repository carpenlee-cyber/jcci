...
# ==================== 参数缓存管理 ====================
def get_params_cache_file() -> str:
    """获取参数缓存文件路径"""
    cache_dir = os.path.join(os.path.dirname(__file__), '.cache')
    if not os.path.exists(cache_dir):
        os.makedirs(cache_dir)
    return os.path.join(cache_dir, 'last_params.json')


def save_params_to_cache(params: Dict) -> bool:
    """
    保存参数到本地缓存文件
    ...
    """
    try:
        cache_file = get_params_cache_file()
        with open(cache_file, 'w', encoding='utf-8') as f:
            json.dump(params, f, ensure_ascii=False, indent=2)
        logger.info(f"参数已缓存到: {cache_file}")
        return True
    except Exception as e:
        logger.error(f"保存参数缓存失败: {e}")
        return False


def load_params_from_cache() -> Optional[Dict]:
    """
    从本地缓存文件加载参数
    
    Returns:
        Dict or None: 缓存的参数，如果不存在则返回None
    """
    try:
        cache_file = get_params_cache_file()
        ...
        with open(cache_file, 'r', encoding='utf-8') as f:
            params = json.load(f)
        logger.info(f"已从缓存加载参数: {cache_file}")
        return params
    except Exception as e:
        logger.error(f"加载参数缓存失败: {e}")
        return None
...
def render_task_submission_page():
    ...
    # 尝试从缓存加载参数
    cached_params = load_params_from_cache()
    
    # 初始化 session state 用于保存表单值
    if 'last_git_url' not in st.session_state:
        if cached_params and 'git_url' in cached_params:
            st.session_state.last_git_url = cached_params['git_url']
        else:
            st.session_state.last_git_url = "https://github.com/carpenlee-cyber/mall.git"
    
    if 'last_username' not in st.session_state:
        if cached_params and 'username' in cached_params:
            st.session_state.last_username = cached_params['username']
        else:
            st.session_state.last_username = "carpenlee-cyber"
    
    if 'last_tag_old' not in st.session_state:
        if cached_params and 'tag_old' in cached_params:
            st.session_state.last_tag_old = cached_params['tag_old']
        else:
            st.session_state.last_tag_old = "baseline_20260508_01"
    
    if 'last_tag_new' not in st.session_state:
        if cached_params and 'tag_new' in cached_params:
            st.session_state.last_tag_new = cached_params['tag_new']
        else:
            st.session_state.last_tag_new = "baseline_fix1_20260508_02"
    
    if 'last_max_depth' not in st.session_state:
        if cached_params and 'max_depth' in cached_params:
            st.session_state.last_max_depth = cached_params['max_depth']
        else:
            st.session_state.last_max_depth = 5
    
    # 注意：出于安全考虑，不缓存 Git 密码
    ...
    if submitted:
        ...
        try:
            ...
            # 同时保存到本地缓存文件（页面刷新后也能恢复）
            # ⚠️ 注意：出于安全考虑，不缓存 Git 密码
            cache_params = {
                'git_url': git_url,
                'username': username,
                'tag_old': tag_old,
