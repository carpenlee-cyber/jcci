    ...
    if submitted:
        ...
        try:
            ...
            if result_url:
                ...
                st.markdown(f"**🌐 Web 访问链接**: [{result_url}]({result_url})")
                ...
                页面将在 **2秒后** 自动跳转到任务列表，您也可以手动点击下方按钮。
                """)
                
                # 提供手动跳转按钮
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("📋 立即查看任务列表", use_container_width=True, key="manual_jump_to_tasks"):
                        navigate_to('tasks')
                with col2:
                    if st.button("🔄 刷新当前页", use_container_width=True, key="refresh_page"):
                        st.rerun()
...
def main():
    ...
    with tab_results:
        ...
        if not baseline_param:
            ...
            - 使用Git Tag: `http://localhost:8501/?baseline=baseline_20260508_01`
