...
class JCCI(object):
    ...
    def analyze_two_commit_incremental(self, commit_new, commit_old, **kwargs):
        ...
        # 生成短标识符用于数据库和文件命名
        self.commit_short_new = _normalize_commit_or_tag(commit_new)
        self.commit_short_old = _normalize_commit_or_tag(commit_old)
        ...
        if is_first_run:
            ...
            if is_duplicate:
                ...
                if cached_result:
                    ...
                    # ✅ 添加源代码目录路径（供 MyBatis Mapper 索引使用）
                    cached_result['source_dir'] = self.file_path
        ...
        # ✅ 添加源代码目录路径（供 MyBatis Mapper 索引使用）
        result['source_dir'] = self.file_path
